...
Aggiungo variabili più leggibili
...
Hybrid estimator of the pcf. If the H0 of CSR in the clark evans test is rejected we use the new pcf estimator made by Baddeley ("a"), otherwise we use the standard estimator corrected near zero.
...
```{r}
map(Lfun_list, \(Lfun_obj) {
  plot(Lfun_obj$fd)
  lines(c(0,350), c(0,350), col = "gold", lwd = 2)
})
```
...
```{r}
fpca_list <- map2(Lfun_list, ppp_pats, \(Lfun_obj, ppp_pat){
  fpca <- pca.fd(Lfun_obj$fd, nharm = 4, Lfun_obj$fdPar)
  rownames(fpca$scores) <- colnames(Lfun_obj$mat)
  fpca
})

fpcaVarmx_list <- map(fpca_list, \(fpca) {varmx.pca.fd(fpca)})

```

## plots of fPCA Varimax rotation

```{r}
fpca_list$`1`$harmonics

map(fpcaVarmx_list, \(fpcaVarmx){plot.pca.fd(fpcaVarmx, cex.main=0.9)})
```
