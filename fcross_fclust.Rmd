...
```{r}
ppp_pats_old <- map(patients_list, function(pat) {
...
```{r}
saveRDS(ppp_pats, file = "dataset/ppp_patients_data.RDS")
```
...
```{r}
Gcross_list <- map(ppp_pats, \(pat){Gest(pat, correction = "best", rmax = 30)})
map(Gcross_list, plot)
```
